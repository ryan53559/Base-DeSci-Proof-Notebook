# Release Checklist / 發布檢查表

## Ready tonight / 今晚可完成

- [x] Base Sepolia smart contract deployed and verified.
...
不收費；建立存證時需在錢包確認一次 Base Sepolia 測試網交易，只會使用測試幣。

## Tomorrow / 明天

- [ ] Record the demo video with `VIDEO_PITCH_ZH_EN.md` and `DEMO_RUNBOOK_ZH_EN.md`.
- [ ] Upload the video and complete the ETHGlobal submission form.

## External review / 等待外部審核

Chainlink must approve CRE deployment access before the confidential workflow
can be deployed live. The current integration is clearly presented as a local
simulation, not a live service.

Chainlink 必須先核准 CRE 部署權限，機密流程才能正式部署。目前專案會清楚標示為
本機模擬，不會假裝已上線。
